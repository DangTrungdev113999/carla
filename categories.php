<?php
	include 'header.php'
?>
		<!-- End header -->
		<!-- Begin page name -->
		<section class="page-name">
			<div class="container">
				<!-- page-name.jpg -->
				<div class="row">
					<div class="col-md-12">
						<h1>Categories</h1>
						<ul class="bread-crumbs">
							<li><a href="index.php">Home</a></li>
							<li><p>Pages</p></li>
							<li><p>Categories</p></li>
						</ul>
					</div>
				</div>
			</div>
		</section>
		<!-- End page name -->
		<main>
			<!-- Begin categories content -->
			<section class="categories-content">
				<div class="container">
					<div class="row">
						<div class="col-lg-12">
							<div class="categories-grid">
								<div class="grid-item grid-width-1">
									<a href="shop-page.php" class="categories-card">
										<span class="category-info">
											<span class="category-info__text">Hot Sale</span>
											<span class="category-name h2">Winter Sale</span>
										</span>
										<img src="public/img/category-1.jpg" alt="img">
									</a>
								</div>
								<div class="grid-item grid-width-2">
									<a href="shop-page.php" class="categories-card">
										<span class="category-info">
											<span class="category-info__text">Hot Sale</span>
											<span class="category-name h2">Men</span>
										</span>
										<img src="public/img/category-2.jpg" alt="img">
									</a>
								</div>
								<div class="grid-item grid-width-2">
									<a href="shop-page.php" class="categories-card">
										<span class="category-info">
											<span class="category-info__text">Hot Sale</span>
											<span class="category-name h2">Shoes</span>
										</span>
										<img src="public/img/category-3.jpg" alt="img">
									</a>
								</div>
								<div class="grid-item grid-width-2">
									<a href="shop-page.php" class="categories-card">
										<span class="category-info">
											<span class="category-info__text">Hot Sale</span>
											<span class="category-name h2">Bags</span>
										</span>
										<img src="public/img/category-4.jpg" alt="img">
									</a>
								</div>
								<div class="grid-item grid-width-2">
									<a href="shop-page.php" class="categories-card">
										<span class="category-info">
											<span class="category-info__text">Hot Sale</span>
											<span class="category-name h2">Dresses</span>
										</span>
										<img src="public/img/category-5.jpg" alt="img">
									</a>
								</div>
								<div class="grid-item grid-width-1">
									<a href="shop-page.php" class="categories-card">
										<span class="category-info">
											<span class="category-info__text">Hot Sale</span>
											<span class="category-name h2">Woman</span>
										</span>
										<img src="public/img/category-6.jpg" alt="img">
									</a>
								</div>
								<div class="grid-item grid-width-3">
									<a href="shop-page.php" class="categories-card">
										<span class="category-info">
											<span class="category-info__text">Hot Sale</span>
											<span class="category-name h2">Jacets</span>
										</span>
										<img src="public/img/category-7.jpg" alt="img">
									</a>
								</div>
								<div class="grid-item grid-width-4">
									<a href="shop-page.php" class="categories-card">
										<span class="category-info">
											<span class="category-info__text">Hot Sale</span>
											<span class="category-name h2">Accessories</span>
										</span>
										<img src="public/img/category-8.jpg" alt="img">
									</a>
								</div>
								<div class="grid-item grid-width-2">
									<a href="shop-page.php" class="categories-card">
										<span class="category-info">
											<span class="category-info__text">Hot Sale</span>
											<span class="category-name h2">Underwear</span>
										</span>
										<img src="public/img/category-9.jpg" alt="img">
									</a>
								</div>
							</div>
						</div>
					</div>
				</div>
			</section>
			<!-- End categories content -->
		</main>
		<div class="bg-popup"></div>
		<div class="back-top" id="backTop"><p>up!</p></div>
		<!-- Begin footer -->
<?php
	include 'footer.php'
?>